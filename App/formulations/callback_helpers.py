...
def split_consolidated_property_values(formulation, all_property_values, active_div_count, binder_div_count, conductive_div_count):
...
def create_generic_formulation_div_callback(formulation_type: FormulationType) -> callable:
    ...
    def generic_update_formulation_materials(
        ...
        active_dropdown_values, binder_dropdown_values, conductive_dropdown_values = split_consolidated_values(
            all_dropdown_values, active_div_count, binder_div_count, conductive_div_count
        )

        # Get the styles for each material type
        active_div_styles, binder_div_styles, conductive_div_styles = split_consolidated_values(
        ...
        active_weight_fractions, binder_weight_fractions, conductive_weight_fractions = split_consolidated_values(
            all_weight_fraction_values, active_div_count, binder_div_count, conductive_div_count
        )

        # get the slider values for each material type
        active_slider_values, binder_slider_values, conductive_slider_values = split_consolidated_property_values(
            formulation, slider_values, active_div_count, binder_div_count, conductive_div_count
        )

        # get the input values for each material type
        active_input_values, binder_input_values, conductive_input_values = split_consolidated_property_values(
            formulation, input_values, active_div_count, binder_div_count, conductive_div_count
        )

        # If all display is none for any of the viewing styles, return no update
        if any(d.get('display') == 'none' for d in viewing_styles):
            from App.formulations.handlers import create_no_update_response
            return create_no_update_response(formulation_config, active_div_styles, binder_div_styles, conductive_div_styles)

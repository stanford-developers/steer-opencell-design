...
class _JellyRoll(_ElectrodeAssembly, ABC):
    ...
    def __init__(
        ...
        if not isinstance(mandrel, (FlatMandrel, RoundMandrel)):
            raise TypeError(
                f"mandrel must be FlatMandrel or RoundMandrel, got {type(mandrel)}"
            )
    ...
    def _calculate_top_down_coordinates(self) -> None:
        ...
        component_configs = [
            ...
            (
                ...
                "_cathode._current_collector._foil_coordinates",
            ),
        ...
        for spiral_key, coords_attr_path in component_configs:
            ...
            self._calculate_component_top_down_coords(
                spiral_key, coords_attr_path, cathode_is_tab_welded, anode_is_tab_welded
            )

        # Calculate tape coordinates if applicable
        if (
            hasattr(self, "_tape")
            and self._tape is not None
            and self._additional_tape_wraps > 0
        ):
    ...
    def _calculate_tape_top_down_coords(self) -> None:
        ...
        x, y = self.build_square_array(
            -tape_diameter / 2, start_y, tape_diameter, tape_width
        )
    ...
    def _calculate_tab_top_down_coords(self, electrode_type: str) -> None:
        ...
        self._component_top_down_coordinates[f"{electrode_type}_tab"] = np.column_stack(
            (x, y)
        )
    ...
    def _calculate_tape_right_left_coords(self) -> None:
        ...
        y, z = self.build_square_array(
            start_y, -tape_diameter / 2, tape_width, tape_diameter
        )
    ...
    def _calculate_tab_right_left_coords(self, electrode_type: str) -> None:
        ...
        self._component_right_left_coordinates[f"{electrode_type}_tab"] = (
            np.column_stack((y, z))
        )
    ...
    def _calculate_pore_volume(self):
        ...
        self._pore_volume = (
            _cathode_pore_volume
            + _anode_pore_volume
            + _bottom_separator_pore_volume
            + _top_separator_pore_volume
        )
    ...
    def _get_center_point(self) -> Tuple[float, float, float]:
        ...
        x_center = (
            all_spirals[:, X_COORD_COL].max() + all_spirals[:, X_COORD_COL].min()
        ) / 2
        z_center = (
            all_spirals[:, Z_COORD_COL].max() + all_spirals[:, Z_COORD_COL].min()
        ) / 2
    ...
    def _get_electrode_coordinates(self, electrode_type: str) -> Dict[str, np.ndarray]:
        ...
        if electrode_type not in ("cathode", "anode"):
            raise ValueError(
                f"electrode_type must be 'cathode' or 'anode', got {electrode_type}"
            )
        ...
        return {"a_side": a_side_coords, "b_side": b_side_coords}

    def _calculate_inner_interface_area(
        self, cathode_coords: np.ndarray, anode_coords: np.ndarray
    ) -> float:
        ...
        if not np.isnan(cathode_coords).any():
            ...
            cathode_segments = self._extract_coordinate_segments(
                cathode_coords[:, 0], cathode_coords[:, 1], unify_xy=True
            )

        if not np.isnan(anode_coords).any():
            anode_segments = [anode_coords]
        else:
            anode_segments = self._extract_coordinate_segments(
                anode_coords[:, 0], anode_coords[:, 1], unify_xy=True
            )

        intersection_area = 0.0
        for c_segment in cathode_segments:
            for a_segment in anode_segments:
                intersection_area += self.get_coordinate_intersection(
                    c_segment, a_segment
                )

        return intersection_area

    def _calculate_outer_interface_area(
        self, cathode_coords: np.ndarray, anode_coords: np.ndarray
    ) -> float:
        ...
        if not np.isnan(shifted_cathode_coords).any():
            ...
            cathode_segments = self._extract_coordinate_segments(
                shifted_cathode_coords[:, 0],
                shifted_cathode_coords[:, 1],
                unify_xy=True,
            )
    ...
    def _calculate_full_rotation_shift(self) -> float:
        ...
        length_one_rotation_ahead = np.interp(
            full_rotation_angle, cathode_spiral[theta_values, THETA_COL], x_values
        )
    ...
    def _calculate_tape_bulk_properties(self) -> None:
        ...
        if self._tape_length_driver == TapeDriver.JELLY_ROLL_DRIVEN:
            if tape_spiral is None:
                ...
                tape_length = (
                    tape_spiral[:, X_UNWRAPPED_COL].max()
                    - tape_spiral[:, X_UNWRAPPED_COL].min()
                ) * M_TO_MM
            ...
            if tape_spiral is None:
                ...
                tape_wraps = (
                    tape_spiral[:, TURNS_COL].max() - tape_spiral[:, TURNS_COL].min()
                )
    ...
    def _translate_spirals_xz(
        self, x_shift: float, z_shift: float
        ...
        self._spiral = SpiralCalculator.translate_spirals_xz(
            self._spiral, x_shift, z_shift
        )
        self._component_spirals = SpiralCalculator.translate_spirals_xz(
            self._component_spirals, x_shift, z_shift
        )
        self._extruded_spirals = SpiralCalculator.translate_spirals_xz(
            self._extruded_spirals, x_shift, z_shift
        )
    ...
    def _translate_right_left_coordinates(
        self, y_shift: float, z_shift: float
    ) -> Dict[str, np.ndarray]:
    ...
    def position_layup_on_mandrel(
        layup: Laminate, mandrel: Union[FlatMandrel, RoundMandrel]
    ) -> Laminate:
        ...
        cathode_b_side_coated_z = cathode_b_side_coated_z[
            ~np.isnan(cathode_b_side_coated_z)
        ]
    ...
    @abstractmethod
    def _calculate_tape_spiral(
        self, tape_params: Dict[str, Any], **kwargs
    ) -> np.ndarray:
    ...
    @abstractmethod
    def _build_tape_extruded_spiral(
        self, tape_spiral: np.ndarray
    ) -> Dict[str, np.ndarray]:
    ...
    def _calculate_roll_properties(self) -> Dict[str, float]:
        ...
        if not is_anode_free:
            ...
            missing_components = [
                comp for comp in required_components if comp not in component_spirals
            ]
        ...
        for sep_name in ["bottom_separator", "top_separator"]:
            ...
            inner_turns = (
                np.nanmax(separator_spiral[inner_mask, TURNS_COL])
            ...
            if np.any(outer_mask):
                ...
                outer_turns = (
                    np.nanmax(outer_spiral) - np.nanmin(outer_spiral)
    ...
    def _calculate_mass_properties(self) -> None:
        ...
        self._mass = (
            self._layup._anode._mass
            + self._layup._cathode._mass
            + sum(s._mass for s in separators if s._mass is not None)
        )
    ...
    def _calculate_cost_properties(self) -> None:
        ...
        self._cost = (
            self._layup._anode._cost
            + self._layup._cathode._cost
            + sum(s._cost for s in separators if s._cost is not None)
        )
    ...
    def _format_spiral_trace(
        self, property_name: str, color: str, name: str, line_width: int = 0
    ) -> Optional[go.Scatter]:
    ...
    def _format_extruded_spiral_trace(
        self, property_name: str, color: str, name: str
    ) -> Optional[go.Scatter]:
    ...
    def _get_cathode_tab_y_position(self) -> float:
        ...
        if type(self._layup._cathode._current_collector) == TabWeldedCurrentCollector:
            cathode_tab_deduction = (
                -self._layup._cathode._current_collector._tab_overhang
            )
        else:
            cathode_tab_deduction = (
    ...
    def roll_properties(self) -> pd.DataFrame:
        ...
        additional_components = [
            comp for comp in df.index if comp not in component_order
        ]
    ...
    def top_separator_spiral_trace(self) -> Optional[go.Scatter]:
        return self._format_spiral_trace(
            "top_separator_spiral",
            self._layup.top_separator.material._color,
            f"Top Separator",
        )
    ...
    def tape_spiral_trace(self) -> Optional[go.Scatter]:
        return self._format_spiral_trace(
            "tape_spiral", self._tape._material._color, f"Tape"
        )

    @property
    def bottom_separator_spiral_trace(self) -> Optional[go.Scatter]:
        return self._format_spiral_trace(
            "bottom_separator_spiral",
            self._layup.bottom_separator.material._color,
            f"Bottom Separator",
        )
    ...
    def anode_a_side_coating_spiral_trace(self) -> Optional[go.Scatter]:
        if self._layup._anode._is_anode_free:  # anode-free: no coating to visualise
            return None
        return self._format_spiral_trace(
            "anode_a_side_coating_spiral",
            self._layup._anode.formulation._color,
            f"Anode a-side Coating",
        )

    @property
    def anode_current_collector_spiral_trace(self) -> Optional[go.Scatter]:
        return self._format_spiral_trace(
            "anode_current_collector_spiral",
            self._layup._anode.current_collector.material._color,
            f"Anode Current Collector",
        )
    ...
    def anode_b_side_coating_spiral_trace(self) -> Optional[go.Scatter]:
        if self._layup._anode._is_anode_free:
            return None
        return self._format_spiral_trace(
            "anode_b_side_coating_spiral",
            self._layup._anode.formulation._color,
            f"Anode b-side Coating",
        )

    @property
    def cathode_a_side_coating_spiral_trace(self) -> Optional[go.Scatter]:
        return self._format_spiral_trace(
            "cathode_a_side_coating_spiral",
            self._layup._cathode.formulation._color,
            f"Cathode a-side Coating",
        )

    @property
    def cathode_current_collector_spiral_trace(self) -> Optional[go.Scatter]:
        return self._format_spiral_trace(
            "cathode_current_collector_spiral",
            self._layup._cathode.current_collector.material._color,
            f"Cathode Current Collector",
        )

    @property
    def cathode_b_side_coating_spiral_trace(self) -> Optional[go.Scatter]:
        return self._format_spiral_trace(
            "cathode_b_side_coating_spiral",
            self._layup._cathode.formulation._color,
            f"Cathode b-side Coating",
        )

    @property
    def tape_extruded_spiral_trace(self) -> Optional[go.Scatter]:
        return self._format_extruded_spiral_trace(
            "tape_extruded_spiral", self._tape._material._color, f"Tape"
        )

    @property
    def top_separator_extruded_spiral_trace(self) -> Optional[go.Scatter]:
        return self._format_extruded_spiral_trace(
            "top_separator_extruded_spiral",
            self._layup.top_separator.material._color,
            f"Top Separator",
        )
    ...
    def bottom_separator_extruded_spiral_trace(self) -> Optional[go.Scatter]:
        return self._format_extruded_spiral_trace(
            "bottom_separator_extruded_spiral",
            self._layup.bottom_separator.material._color,
            f"Bottom Separator",
        )
    ...
    def anode_a_side_coating_extruded_spiral_trace(self) -> Optional[go.Scatter]:
        if self._layup._anode._is_anode_free:
            return None
        return self._format_extruded_spiral_trace(
            "anode_a_side_coating_extruded_spiral",
            self._layup._anode._formulation._color,
            f"Anode a-side Coating",
        )
    ...
    def anode_current_collector_extruded_spiral_trace(self) -> Optional[go.Scatter]:
        return self._format_extruded_spiral_trace(
            "anode_current_collector_extruded_spiral",
            self._layup._anode.current_collector.material._color,
            f"Anode Current Collector",
        )
    ...
    def anode_b_side_coating_extruded_spiral_trace(self) -> Optional[go.Scatter]:
        if self._layup._anode._is_anode_free:
            return None
        return self._format_extruded_spiral_trace(
            "anode_b_side_coating_extruded_spiral",
            self._layup._anode.formulation._color,
            f"Anode b-side Coating",
        )
    ...
    def cathode_a_side_coating_extruded_spiral_trace(self) -> Optional[go.Scatter]:
        return self._format_extruded_spiral_trace(
            "cathode_a_side_coating_extruded_spiral",
            self._layup._cathode.formulation._color,
            f"Cathode a-side Coating",
        )
    ...
    def cathode_current_collector_extruded_spiral_trace(self) -> Optional[go.Scatter]:
        return self._format_extruded_spiral_trace(
            "cathode_current_collector_extruded_spiral",
            self._layup._cathode.current_collector.material._color,
            f"Cathode Current Collector",
        )
    ...
    def cathode_b_side_coating_extruded_spiral_trace(self) -> Optional[go.Scatter]:
        return self._format_extruded_spiral_trace(
            "cathode_b_side_coating_extruded_spiral",
            self._layup._cathode.formulation._color,
            f"Cathode b-side Coating",
        )
    ...
    def tape_length_driver(self, value: TapeDriver) -> None:
        ...
        if not isinstance(value, TapeDriver):
            raise TypeError(
                f"tape_length_driver must be TapeDriver enum, got {type(value)}"
            )
    ...
    def tape(self, value: Optional[Tape]) -> None:
        ...
        if (
            value._width is None
            or value._width < self._layup._anode._current_collector._y_foil_length
        ):
...
class WoundJellyRoll(_JellyRoll):
    ...
    def _set_main_dimensions_for_objective(self) -> None:
        """Cheap subset of ``_calculate_geometry_parameters`` for Brent loops.

        Skips:
          * ``_calculate_radius_range`` — its inputs (mandrel radius, layup
            length range, layup geometry) are invariant across a single
            radius-setter call, so it produces the same answer every iteration.
          * ``_calculate_total_height`` — reads from
            ``_component_top_down_coordinates``, which the setter never
    ...
    def _calculate_radius_range(self) -> Tuple[float, float]:
        ...
        small_spiral = SpiralCalculator.calculate_variable_thickness_spiral(
            small_layup, self._mandrel._radius
        )
        small_spiral_coords_2d = np.column_stack(
            (small_spiral[:, X_COORD_COL], small_spiral[:, Z_COORD_COL])
        )
        small_radius, _ = self.get_radius_of_points(small_spiral_coords_2d)
        small_radius = small_radius + small_layup.get_thickness_at_x(
            small_layup._total_length
        )
        ...
        big_spiral = SpiralCalculator.calculate_variable_thickness_spiral(
            big_layup, self._mandrel._radius
        )
        big_spiral_coords_2d = np.column_stack(
            (big_spiral[:, X_COORD_COL], big_spiral[:, Z_COORD_COL])
        )
    ...
    def _calculate_variable_thickness_spiral(self, **kwargs) -> np.ndarray:
        ...
        **kwargs
        ...
        self._spiral = SpiralCalculator.calculate_variable_thickness_spiral(
            laminate=self._layup, start_radius=self._mandrel._radius, **kwargs
        )
    ...
    def _get_high_resolution_params(self) -> Dict[str, Any]:
        ...
        """
        return {"dtheta": 0.1}  # 0.1° angular spacing for spiral calculation
...
class FlatWoundJellyRoll(_JellyRoll):
    ...
    def __init__(
        ...
        cathode_notch_alignment_angle: Optional[float] = None,
        anode_notch_alignment_angle: Optional[float] = None,
    ) -> None:
        ...
        mandrel : FlatMandrel
            Flat mandrel for racetrack winding
        cathode_notch_alignment_angle : float, optional
            Winding phase in radians at which cathode notch centers align.
        anode_notch_alignment_angle : float, optional
            Winding phase in radians at which anode notch centers align.
        ...
        self._cathode_notch_alignment_angle = self._validate_notch_alignment_angle(
            cathode_notch_alignment_angle, "cathode_notch_alignment_angle"
        )
        self._anode_notch_alignment_angle = self._validate_notch_alignment_angle(
            anode_notch_alignment_angle, "anode_notch_alignment_angle"
        )
    ...
    @staticmethod
    def _validate_notch_alignment_angle(
        value: Optional[float], name: str
    ) -> Optional[float]:
        """Validate an optional notch-stack phase angle in radians."""
        if value is None:
            return None
        try:
            angle = float(value)
    ...
    def _apply_thickness_aware_notches(self) -> None:
        """Apply configured same-phase notch centers to notched collectors.

        The component spiral supplies the authoritative mapping between winding
        angle and unwrapped sheet length. Collector coordinates are refreshed
        directly to avoid recursively invoking parent propagation while the
        jelly roll itself is being calculated.
        """
        generated = set(self._thickness_aware_notch_electrodes)
        configurations = {
            "cathode": self._cathode_notch_alignment_angle,
            "anode": self._anode_notch_alignment_angle,
        }

        for electrode_name, alignment_angle in configurations.items():
            ...
            if not isinstance(collector, NotchedCurrentCollector) or isinstance(
                ...
                raise TypeError(
                    f"{electrode_name}_notch_alignment_angle requires a "
                    "NotchedCurrentCollector."
                )

            spiral = self._component_spirals[f"{electrode_name}_current_collector"]
            centers_global = SpiralCalculator.aligned_positions_from_spiral(
                spiral=spiral,
    ...
    def _clear_generated_notches_on_layup(self, layup: Laminate) -> None:
        """Clear assembly-generated patterns before changing a copied layup length."""
        configurations = {
            "cathode": self._cathode_notch_alignment_angle,
            "anode": self._anode_notch_alignment_angle,
        }
    ...
    def _apply_tape_pre_centering_transforms(self, tape_params: Dict[str, Any]) -> None:
        ...
        self._spiral, self._component_spirals, self._extruded_spirals = (
            self._translate_spirals_xz(
                x_shift=-center_x, z_shift=-center_z + thickness / 2
            )
        )
    ...
    def _set_main_dimensions_for_objective(self) -> None:
        ...
        Skips ``_calculate_thickness_width_range`` (which would re-run the
        full racetrack pipeline twice — at the layup length bounds — even
        though those bounds are invariant across a single Brent loop) and
        ``_calculate_total_height`` (reads from a shallow-copied dict that
        the objective never refreshes).
    ...
    def _calculate_thickness_width_range(
        self,
        ...
        mandrel_circumference = 2 * (
            TWO_PI * self._mandrel._radius + 2 * self._mandrel._straight_length
        )
        min_layup_length = max(mandrel_circumference, self._layup.length_range[0])
        ...
        small_layup = deepcopy(self._layup)
        self._clear_generated_notches_on_layup(small_layup)
        small_layup.length = min_layup_length
        small_layup = self.position_layup_on_mandrel(small_layup, self._mandrel)
        small_layup.calculate_flattened_center_lines()
        small_pressed_racetrack = (
            SpiralCalculator.calculate_variable_thickness_racetrack(
                small_layup, radius, straight_length
            )
        )
        small_racetrack_coords_2d = np.column_stack(
            (
                small_pressed_racetrack[:, X_COORD_COL],
                small_pressed_racetrack[:, Z_COORD_COL],
            )
        )
        small_width = SpiralCalculator.get_width_of_racetrack(small_racetrack_coords_2d)
        small_width = small_width + 2 * small_layup.get_thickness_at_x(
            small_layup._total_length
        )
        small_thickness = SpiralCalculator.get_thickness_of_racetrack(
            small_racetrack_coords_2d
        )
        small_thickness = small_thickness + 2 * small_layup.get_thickness_at_x(
            small_layup._total_length
        )
        ...
        big_layup = self.position_layup_on_mandrel(big_layup, self._mandrel)
        big_layup.calculate_flattened_center_lines()
        big_pressed_racetrack = SpiralCalculator.calculate_variable_thickness_racetrack(
            big_layup, radius, straight_length
        )
        big_racetrack_coords_2d = np.column_stack(
            (
                big_pressed_racetrack[:, X_COORD_COL],
                big_pressed_racetrack[:, Z_COORD_COL],
            )
        )
        big_width = SpiralCalculator.get_width_of_racetrack(big_racetrack_coords_2d)
        big_width = big_width + 2 * big_layup.get_thickness_at_x(
            big_layup._total_length
        )
        big_thickness = SpiralCalculator.get_thickness_of_racetrack(
            big_racetrack_coords_2d
        )
        big_thickness = big_thickness + 2 * big_layup.get_thickness_at_x(
            big_layup._total_length
        )
    ...
    def _calculate_pressed_racetrack(
        self, pressed_height: float = DEFAULT_PRESSED_HEIGHT
    ) -> Tuple[float, float]:
        ...
        original_circumference = (
            2 * np.pi * original_radius + 2 * original_straight_length
        )
    ...
    def _center_spirals(self) -> None:
        ...
        spirals_x_z = spirals_x_z[
            ~np.isnan(spirals_x_z).any(axis=1)
    ...
    def _rotate_spirals_to_minimize_thickness(
        self,
        use_extruded: bool = True,
        ...
        initial_angle : float, optional
            Warm-start angle for the inner Brent search. Forwarded to
            ``SpiralCalculator.rotate_spiral_to_minimize_thickness``; see that
            function's docstring for the search-strategy details.
        ...
        if (
            use_extruded
            and hasattr(self, "_extruded_spirals")
            and self._extruded_spirals
        ):
        ...
        if hasattr(self, "_component_spirals") and isinstance(
            self._component_spirals, dict
        ):
            all_spirals.update(
                {f"comp_{k}": v for k, v in self._component_spirals.items()}
            )
        if hasattr(self, "_extruded_spirals") and isinstance(
            self._extruded_spirals, dict
        ):
            all_spirals.update(
    ...
    def _get_high_resolution_params(self) -> Dict[str, Any]:
        ...
        """
        return {"ds_target": 0.0002}  # 0.2mm target spacing for racetrack calculation
    ...
    def cathode_notch_alignment_angle(self) -> Optional[float]:
        """Return the cathode notch-stack phase angle in radians."""
        return self._cathode_notch_alignment_angle

    @cathode_notch_alignment_angle.setter
    @calculate_all_properties
    def cathode_notch_alignment_angle(self, value: Optional[float]) -> None:
        self._cathode_notch_alignment_angle = self._validate_notch_alignment_angle(
            value, "cathode_notch_alignment_angle"
        )

    @property
    def anode_notch_alignment_angle(self) -> Optional[float]:
        """Return the anode notch-stack phase angle in radians."""
        return self._anode_notch_alignment_angle

    @anode_notch_alignment_angle.setter
    @calculate_all_properties
    def anode_notch_alignment_angle(self, value: Optional[float]) -> None:
        self._anode_notch_alignment_angle = self._validate_notch_alignment_angle(
            value, "anode_notch_alignment_angle"
        )
    ...
    def plot_notch_alignment(self, layered: bool = False, **kwargs: Any) -> go.Figure:
        ...
        Markers at increasing radii should form one radial stack for each
        configured electrode, demonstrating that every notch has the same
        winding phase.
        """
    ...
    def thickness(self, target_thickness: float) -> None:
        ...
        # final ``self.layup = self._layup`` reassignment after Brent.
        template_layup = deepcopy(self._layup)
        self._clear_generated_notches_on_layup(template_layup)

...
class _Cell:
    def __init__(
        ...
        self._check_and_copy_electrode_assembly(
            electrode_assembly, n_electrode_assembly
        )
    ...
    def _calculate_electrolyte_quantities(self):
        ...
        """
        self._electrolyte._volume = sum(
            [s._pore_volume for s in self._electrode_assemblies]
        ) * (1 + self._electrolyte_overfill)
        self._electrolyte._mass = self._electrolyte._volume * self._electrolyte._density
        self._electrolyte._cost = (
            self._electrolyte._mass
        ) * self._electrolyte._specific_cost

    def _check_anode_free(
        self, electrode_assembly: Stack | CylindricalJellyRoll | FlatJellyRoll
    ) -> None:
    ...
    def _check_and_copy_electrode_assembly(
        ...
        if not isinstance(
            ...
            raise ValueError(
                "Electrode assembly must be an instance of Stack, CylindricalJellyRoll or FlatJellyRoll"
            )
        ...
        self._electrode_assemblies = [
            deepcopy(electrode_assembly) for _ in range(n_electrode_assembly)
        ]

    def _check_and_copy_electrolyte(
        self, electrolyte: Electrolyte, electrolyte_overfill: float
    ):
        """
        ...
        if not (0 <= electrolyte_overfill <= 100):
            raise ValueError(
                "Electrolyte_overfill percentage must be between 0 and 100"
            )

        self._electrolyte = deepcopy(electrolyte)
        self._electrolyte_overfill = electrolyte_overfill / 100

    def _check_and_copy_encapsulation(
        self, encapsulation: Pouch | PrismaticCase | CylindricalCase
    ) -> None:
        """
        ...
        if not isinstance(encapsulation, (Pouch, PrismaticCase, CylindricalCase)):
            raise ValueError(
                "Encapsulation must be an instance of Pouch, PrismaticCase or CylindricalShell"
            )
    ...
    def _calculate_half_cell_curve(self):
        ...
        if not self._anode_free:
            anode_half_cell = (
                pd.concat(
                    [s._anode_half_cell_curve for s in self._electrode_assemblies]
                )
                .groupby(["direction", "voltage"], as_index=False)["capacity"]
                .sum()
                .groupby("direction", as_index=False)
                .apply(
                    lambda x: x.sort_values(
                        "capacity",
                        ascending=True
                        if x["direction"].values[0] == "charge"
                        else False,
                    )
                )
            )
    ...
    def _calculate_full_cell_curve(self):
        ...
        full_cell_curve = (
            pd.concat([s._full_cell_curve for s in self._electrode_assemblies])
            .groupby(["direction", "voltage"], as_index=False)["capacity"]
            .sum()
        ...
        max_cap = (
            top_cap
            if top_cap < charge_curve["capacity"].max()
            else charge_curve["capacity"].max()
        )
        ...
        charge_curve = pd.DataFrame(
            {"capacity": cap_grid, "voltage": new_voltage, "direction": "charge"}
        )
        ...
        min_cap = (
            self._irreversible_capacity
            if self._irreversible_capacity > discharge_curve["capacity"].min()
            else discharge_curve["capacity"].min()
        )
        cap_grid = np.linspace(min_cap, max_cap, self._grid_n)
        cs = CubicSpline(discharge_curve["capacity"], discharge_curve["voltage"])
        new_voltage = cs(cap_grid)
        discharge_curve = pd.DataFrame(
            {"capacity": cap_grid, "voltage": new_voltage, "direction": "discharge"}
        )
    ...
    def _get_effective_areal_capacity(self) -> None:
        ...
        """
        self._effective_areal_capacity = self._reversible_capacity / sum(
            [s._areal_capacity for s in self._electrode_assemblies]
        )
    ...
    def _linear_interpolate_on_voltage(self, df) -> pd.DataFrame:
        ...
        return pd.DataFrame({"capacity": y_new, "voltage": self._v_grid}).query(
            "voltage >= @x_min and voltage <= @x_max"
        )
    ...
    def _linear_interpolate_on_capacity(self, df) -> pd.DataFrame:
        ...
        return pd.DataFrame({"voltage": y_new, "capacity": self._c_grid}).query(
            "capacity >= @x_min and capacity <= @x_max"
        )
    ...
    def _get_color_map(self):
        ...
        for item, color_tup in zip(items, color_tups):
            anode_items = [
                a.name for a in list(self._anode_mass_breakdown[item].keys())
            ]
            ...
            cathode_items = [
                c.name for c in list(self._cathode_mass_breakdown[item].keys())
            ]
            cathode_colors = get_colorway(
                color_tup[0], color_tup[1], len(cathode_items)
            )
            cathode_map = {
                item: color for item, color in zip(cathode_items, cathode_colors)
            }
    ...
    def _get_mass_breakdown_plot_pie(self, **kwargs):
        ...
        mass_breakdown = pd.DataFrame(
            mass_breakdown.items(), columns=["component", "mass"]
        ).assign(level="Cell")

        anode_mass_breakdown = self.anode_mass_breakdown
        anode_mass_breakdown = {
            obj: value
            for innder_dict in anode_mass_breakdown.values()
            for obj, value in innder_dict.items()
        }
        anode_mass_breakdown = (
            pd.DataFrame(anode_mass_breakdown.items(), columns=["component", "mass"])
            .assign(level="Anode")
        ...
        cathode_mass_breakdown = {
            obj: value
            for innder_dict in cathode_mass_breakdown.values()
            for obj, value in innder_dict.items()
        }
        cathode_mass_breakdown = (
            pd.DataFrame(cathode_mass_breakdown.items(), columns=["component", "mass"])
    ...
    def _get_cost_breakdown_plot_pie(self, **kwargs):
        ...
        cost_breakdown = pd.DataFrame(
            cost_breakdown.items(), columns=["component", "cost"]
        ).assign(level="Cell")

        anode_cost_breakdown = self.anode_cost_breakdown
        anode_cost_breakdown = {
            obj: value
            for innder_dict in anode_cost_breakdown.values()
            for obj, value in innder_dict.items()
        }
        anode_cost_breakdown = (
            pd.DataFrame(anode_cost_breakdown.items(), columns=["component", "cost"])
            .assign(level="Anode")
            .assign(component=lambda x: x["component"].apply(lambda y: y.name))
        )
        ...
        cathode_cost_breakdown = {
            obj: value
            for innder_dict in cathode_cost_breakdown.values()
            for obj, value in innder_dict.items()
        }
        cathode_cost_breakdown = (
            pd.DataFrame(cathode_cost_breakdown.items(), columns=["component", "cost"])
            .assign(level="Cathode")
            .assign(component=lambda x: x["component"].apply(lambda y: y.name))
        )
    ...
    def _get_cost_breakdown_plot_sunburst(self, background_color="white", **kwargs):
        cost_breakdown = (
            pd.DataFrame(self.cost_breakdown.items(), columns=["level_1", "cost"])
        ...
        anode_cost_breakdown = (
            pd.DataFrame(anode_cost_breakdown.items(), columns=["level_2", "cost"])
            .assign(level_1="Anode")
            .assign(level_0="Cell")
        ...
        cathode_cost_breakdown = (
            pd.DataFrame(cathode_cost_breakdown.items(), columns=["level_2", "cost"])
            .assign(level_1="Cathode")
            .assign(level_0="Cell")
            .assign(level_2=lambda x: x["level_2"].apply(lambda y: y.name))
        )
    ...
    def _get_mass_breakdown_plot_sunburst(self, background_color="white", **kwargs):
        mass_breakdown = (
            pd.DataFrame(self.mass_breakdown.items(), columns=["level_1", "mass"])
            .query('level_1 != "Electrode Assemblies"')
            .assign(level_2=None)
            .assign(level_0="Cell")
        )
        ...
        anode_mass_breakdown = (
            pd.DataFrame(anode_mass_breakdown.items(), columns=["level_2", "mass"])
            .assign(level_1="Anode")
            .assign(level_0="Cell")
            .assign(level_2=lambda x: x["level_2"].apply(lambda y: y.name))
        )
        ...
        cathode_mass_breakdown = (
            pd.DataFrame(cathode_mass_breakdown.items(), columns=["level_2", "mass"])
            .assign(level_1="Cathode")
            .assign(level_0="Cell")
            .assign(level_2=lambda x: x["level_2"].apply(lambda y: y.name))
        )
    ...
    def _calculate_mass_breakdown(self):
        ...
        self._electrode_assembly_mass_breakdown = {
            "cathode": sum(
                [s._mass_breakdown["cathode"] for s in self._electrode_assemblies]
            ),
            "anode": sum(
                [s._mass_breakdown["anode"] for s in self._electrode_assemblies]
            ),
            "separator": sum(
                [s._mass_breakdown["separator"] for s in self._electrode_assemblies]
            ),
        }
        ...
        for s in self._electrode_assemblies:
            for key in anode_mass_breakdown.keys():
                anode_mass_breakdown[key] = self._add_to_dict(
                    anode_mass_breakdown[key], s._anode_mass_breakdown[key]
                )
            for key in cathode_mass_breakdown.keys():
                cathode_mass_breakdown[key] = self._add_to_dict(
                    cathode_mass_breakdown[key], s._cathode_mass_breakdown[key]
                )
    ...
    def _calculate_cost_breakdown(self):
        ...
        self._electrode_assembly_cost_breakdown = {
            "cathode": sum(
                [s._cost_breakdown["cathode"] for s in self._electrode_assemblies]
            ),
            "anode": sum(
                [s._cost_breakdown["anode"] for s in self._electrode_assemblies]
            ),
            "separator": sum(
                [s._cost_breakdown["separator"] for s in self._electrode_assemblies]
            ),
        }
        ...
        for s in self._electrode_assemblies:
            for key in anode_cost_breakdown.keys():
                anode_cost_breakdown[key] = self._add_to_dict(
                    anode_cost_breakdown[key], s._anode_cost_breakdown[key]
                )
            for key in cathode_cost_breakdown.keys():
                cathode_cost_breakdown[key] = self._add_to_dict(
                    cathode_cost_breakdown[key], s._cathode_cost_breakdown[key]
                )
    ...
    def get_capacity_voltage_plot(
        ...
        **kwargs,
        ...
        anode_curve = (
            self.anode_half_cell_curve.copy().assign(Electrode="Anode")
            if not self._anode_free
            else None
        )
        ...
        if n_p_ratio is not None:
            ...
            x_max = (
                data["Capacity (Ah)"].max()
                if data["Capacity (Ah)"].max() > n_p_ratio_value
                else n_p_ratio_value
            )
        else:
    ...
    def get_mass_breakdown_plot(self, mode: str = "sunburst", **kwargs):
        if mode.lower() == "pie":
            ...
            raise ValueError(
                "Plot mode not recognized. Please choose between ['pie', 'sunburst']"
            )
    ...
    def full_cell_curve(self):
        ...
        data = self._full_cell_curve.assign(
    ...
    def cost_breakdown(self) -> dict:
        ...
        return {
            item.replace("_", " ").title(): round(value, 3)
            for item, value in self._cost_breakdown.items()
        }
    ...
    def electrode_assembly_mass_breakdown(self) -> dict:
        ...
        return {
            item.replace("_", " ").title(): round(value * KG_TO_G, 3)
            for item, value in self._electrode_assembly_mass_breakdown.items()
        }
    ...
    def electrode_assembly_cost_breakdown(self) -> dict:
        ...
        return {
            item.replace("_", " ").title(): round(value, 3)
            for item, value in self._electrode_assembly_cost_breakdown.items()
        }
    ...
    def anode_cost_breakdown(self) -> dict:
        ...
        cost_breakdown = {
            key.replace("_", " ").title(): {
                obj: round(value, 3) for obj, value in inner.items()
            }
    ...
    def stacks_mass_breakdown(self) -> dict:
        ...
        return {
            item.replace("_", " ").title(): round(value * KG_TO_G, 3)
            for item, value in self._stacks_mass_breakdown.items()
        }
    ...
    def anode_mass_breakdown(self) -> dict:
        ...
        mass_breakdown = {
            key.replace("_", " ").title(): {
                obj: round(value * KG_TO_G, 3) for obj, value in inner.items()
            }
    ...
    def effective_areal_capacity(self) -> float:
        return round(
            self._effective_areal_capacity * (S_TO_H * A_TO_mA / M_TO_CM**2), 6
        )
...
class _StackedCell(_Cell):
    ...
    def _check_and_copy_electrode_assembly(
        self, electrode_assembly: Stack, n_electrode_assembly: int
    ) -> None:
        """
        Function to validate the electrode assembly and copy it n times
...
class _JellyRollCell(_Cell):
    ...
    def _check_and_copy_electrode_assembly(
        self, electrode_assembly: _JellyRoll, n_electrode_assembly: int
    ) -> None:
        """
        Function to validate the electrode assembly and copy it n times
...
class StackedPouchCell(_PouchCell, _StackedCell):
    ...
    def _calculate_geometry_properties(self):
        ...
        self._height = (
            sum([s._thickness for s in self._electrode_assemblies])
            + self._encapsulation._laminate._thickness * 2
        )
        self._volume = (
            self._encapsulation._length * self._encapsulation._width * self._height
        )
...
class StackedPrismaticCell(_PrismaticCell, _StackedCell):
    ...
    def _check_encapsulation(
        self, stack: Stack, prismatic_case: PrismaticCase, n_stacks: int
    ) -> None:
        # Check stack is small enough to fit in the prismatic case
        if stack._thickness * n_stacks > prismatic_case._internal_height:
            raise ValueError(
                "Stack thickness cannot be greater than the internal height of the prismatic case"
            )
        if stack._width > prismatic_case._internal_width:
            raise ValueError(
                "Stack width cannot be greater than the internal width of the prismatic case"
            )
        if stack._length > prismatic_case._internal_length:
            raise ValueError(
                "Stack length cannot be greater than the internal length of the prismatic case"
            )
...
class CylindricalCell(_JellyRollCell):
    ...
    def _check_and_copy_electrode_assembly(
        self, electrode_assembly: CylindricalJellyRoll, n_electrode_assembly: int
    ) -> None:
        """
        Function to validate the electrode assembly and copy it n times
        ...
        if electrode_assembly._radius > self._encapsulation._inner_radius:
            raise ValueError(
                f"Electrode assembly, {electrode_assembly.radius} mm,  cannot be greater than the internal radius of the cylindrical case, {self._encapsulation.inner_radius} mm"
            )

        if electrode_assembly._width > self._encapsulation._inner_height:
            raise ValueError(
                f"Electrode assembly, {electrode_assembly.width} mm, cannot be greater than the internal height of the cylindrical case, {self._encapsulation.inner_height} mm"
            )
    ...
    def get_top_down_view(self, background_color="white", **kwargs) -> go.Figure:
        ...
        fig.update_layout(
            ...
            yaxis=dict(
                showgrid=False, zeroline=False, showticklabels=False, title="Y (cm)"
            ),
            paper_bgcolor=background_color,
            plot_bgcolor=background_color,

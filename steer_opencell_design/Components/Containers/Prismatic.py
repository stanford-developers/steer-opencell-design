...
class _PrismaticComponent(
    ...
    def _rotate_x(self, degrees: float = 90) -> None:
        """
    ...
    def _rotate_y(self, degrees: float = 90) -> None:
        """
...
class PrismaticLidAssembly(_PrismaticComponent):
    ...
    The lid has a simple rectangular footprint and sits on top of the cannister.
    """
...
class PrismaticCannister(
    ...
    def __init__(
        ...
        datum: Tuple[float, float, float] = (0.0, 0.0, 0.0),
        name: str = "Prismatic Cannister",
    ):
        """Initialize a prismatic cannister.
        ...
        material : PrismaticContainerMaterial
            Container material for physical properties
        width : float
            Outer width of the cannister in mm. Must be positive.
        length : float
            Outer length of the cannister in mm. Must be positive.
        height : float
            Height of the cannister in mm. Must be positive.
        wall_thickness : float
            Wall thickness in mm. Must be positive and less than half of outer dimensions.
        datum : Tuple[float, float, float], default=(0.0, 0.0, 0.0)
            Center position in mm as (x, y, z) coordinates
        name : str, default="Prismatic Cannister"
            Component identifier name
        """
        self._update_properties = False
        self._rotated_z = False
    ...
    def _calculate_all_properties(self):
        """Calculate all cannister properties."""
        self._calculate_bulk_properties()
        self._calculate_coordinates()

    def _calculate_bulk_properties(self):
        """Calculate volume, mass, and cost of the cannister."""
    ...
    def _get_right_left_coordinates(self):
        ...
        If rotated_z is False: U-shaped profile (bucket shape) facing right (+x direction).
        If rotated_z is True: Rectangular profile with x-axis as width, y-axis as length.
        """
        if not self._rotated_z:
            # Bucket shape (U-shape) facing right - opening towards +x direction
            half_length = self._length / 2
            
            # Define the U-shape profile points (clockwise from bottom-left)
            # Bottom-left (back-bottom) outer corner
            x1 = self._datum[0]
            y1 = self._datum[1] - half_length
            
            # Bottom-left (back-bottom) inner corner  
            x2 = self._datum[0] + self._wall_thickness
            y2 = self._datum[1] - half_length + self._wall_thickness
            
            # Top-left (back-top) inner corner
            x3 = self._datum[0] + self._height
            y3 = self._datum[1] - half_length + self._wall_thickness
            
            # Top-right (front-top) inner corner
            x4 = self._datum[0] + self._height
            y4 = self._datum[1] + half_length - self._wall_thickness
            
            # Bottom-right (front-bottom) inner corner
            x5 = self._datum[0] + self._wall_thickness
            y5 = self._datum[1] + half_length - self._wall_thickness
            
            # Bottom-right (front-bottom) outer corner
            x6 = self._datum[0]
            y6 = self._datum[1] + half_length
            
            # Top-right (front-top) outer corner
            x7 = self._datum[0] + self._height
            y7 = self._datum[1] + half_length
            
            # Top-left (back-top) outer corner
            x8 = self._datum[0] + self._height
            y8 = self._datum[1] - half_length
            
            # Create the U-shaped profile facing right
            x_coords = np.array([x1, x8, x3, x2, x5, x4, x7, x6, x1])
            y_coords = np.array([y1, y8, y3, y2, y5, y4, y7, y6, y1])
        else:
    ...
    #     """Calculate the top-down view coordinates showing rectangular frame."""
    #     half_width = self._outer_width / 2
    #     half_length = self._outer_length / 2
    ...
    #     # Inner rectangle (counter-clockwise to create hole)
    #     inner_half_width = self._inner_width / 2
    #     inner_half_length = self._inner_length / 2
        
    #     x_inner = np.array([
    #         self._datum[0] - inner_half_width,
    #         self._datum[0] - inner_half_width,
    #         self._datum[0] + inner_half_width,
    #         self._datum[0] + inner_half_width,
    #         self._datum[0] - inner_half_width,
    ...
    #     z_inner = np.array([
    #         self._datum[2] - inner_half_length,
    #         self._datum[2] + inner_half_length,
    #         self._datum[2] + inner_half_length,
    #         self._datum[2] - inner_half_length,
    #         self._datum[2] - inner_half_length,
    ...
    #     self._top_down_coordinates = np.vstack((
    #         np.column_stack((x_outer, z_outer)),
    #         np.column_stack((x_inner, z_inner))
    #     ))

    def get_top_down_view(self, **kwargs) -> go.Figure:
        """Generate a top-down view plot of the cannister."""
        figure = go.Figure()
        figure.add_trace(self.top_down_trace)
    ...
    def get_right_left_view(self, **kwargs) -> go.Figure:
        """Generate a right-left view plot of the cannister."""
        figure = go.Figure()
        figure.add_trace(self.right_left_trace)
    ...
    def _rotate_z(self, degrees: float = -90) -> None:
        """
        Rotate the cannister around the z-axis by the specified degrees.
    ...
    def inner_width(self) -> float:
        """Inner width of the cannister in mm."""
        return round(self._inner_width * M_TO_MM, 2)
    
    @property
    def inner_length(self) -> float:
        """Inner length of the cannister in mm."""
        return round(self._inner_length * M_TO_MM, 2)
    ...
    def inner_height(self) -> float:
        """Inner height of the cannister in mm."""
    ...
    @property
    def rotated_z(self) -> bool:
        """Whether the cannister has been rotated around the z-axis."""
        return self._rotated_z
    ...
    def rotated_z(self, value: bool) -> None:
        ...
        if value and not self._rotated_z:
            ...
            self._rotate_z(degrees=90)  # Note: PrismaticCannister uses -90 as default, so +90 unrotates
            self._rotated_z = False
            self._get_right_left_coordinates()  # Recalculate after rotation state changes
...
class PrismaticEncapsulation(_Container):
    ...
    This class combines a rectangular cannister with a lid assembly and two
    ...
    def _calculate_coordinates(self):
        """Position all components relative to the cannister."""
        self._calculate_dimensions()
        self._position_cannister()
    ...
    def _position_cannister(self):
        """Position and rotate cannister based on connector orientation."""
        # Set cannister at encapsulation datum
        self._cannister.datum = (
            self._datum[0] * M_TO_MM,
            self._datum[1] * M_TO_MM,
            self._datum[2] * M_TO_MM
        )
        
        # Rotate cannister based on connector orientation
        if self._connector_orientation == ConnectorOrientation.LONGITUDINAL:         
            self._cannister.rotated_z = False
        elif self._connector_orientation == ConnectorOrientation.TRANSVERSE:
            self._cannister.rotated_z = True
    ...
    def _position_lid_longitudinal(self):
        ...
        self._lid_assembly.datum = (
            self._cannister._datum[0] * M_TO_MM,
            (self._cannister._datum[1] + self._cannister._height - self._lid_assembly._thickness / 2) * M_TO_MM,
            self._cannister._datum[2] * M_TO_MM
        )
    ...
    def _position_lid_transverse(self):
        ...
        self._lid_assembly.datum = (
            (self._cannister._datum[0] + self._cannister._height - self._lid_assembly._thickness / 2) * M_TO_MM,
            self._cannister._datum[1] * M_TO_MM,
            self._cannister._datum[2] * M_TO_MM
        )
    ...
    def _position_cathode_longitudinal(self):
        ...
        if self._cathode_terminal_connector_position is not None:
            ...
            left_edge = self._cannister._datum[0] - self._cannister._inner_width / 2
            x_position = left_edge + self._cathode_terminal_connector_position * MM_TO_M
        else:
            # Use default centered position (1/4 from left)
            cathode_offset = self._cannister._inner_width / 4
            x_position = self._cannister._datum[0] - cathode_offset
        
        y_position = (self._cannister._datum[1] + self._cannister._height - 
    ...
    def _position_cathode_transverse(self):
        ...
        x_position = (self._cannister._datum[0] + self._cannister._height - 
                     self._lid_assembly._thickness - 
                     self._cathode_terminal_connector._length / 2)
        y_position = (self._cannister._datum[1] + self._cannister._inner_width / 2 - 
                     self._cathode_terminal_connector._thickness / 2)
    ...
    def _position_anode_longitudinal(self):
        ...
        if self._anode_terminal_connector_position is not None:
            ...
            anode_offset = self._cannister._inner_width / 4
            x_position = self._cannister._datum[0] + anode_offset
    ...
    def _center_cavity_at_datum(self):
        ...
        if self._connector_orientation == ConnectorOrientation.LONGITUDINAL:
            ...
            cavity_center_x = self._cannister._datum[0]
            cavity_center_y = (self._cannister._datum[1] + self._cannister._wall_thickness + 
                             self._internal_height / 2)
            cavity_center_z = self._cannister._datum[2]
        elif self._connector_orientation == ConnectorOrientation.TRANSVERSE:
            # For transverse, the cavity center calculation differs due to rotation
            cavity_center_x = self._cannister._datum[0] + self._cannister._wall_thickness + self._internal_width / 2
            cavity_center_y = self._cannister._datum[1]
            cavity_center_z = self._cannister._datum[2]
    ...
    def _calculate_dimensions(self):
        ...
        _min_cannister_base = self._cannister._datum[1] + self._cannister._wall_thickness
        self._mid_y_point = (_max_connector_bottom + _min_cannister_base) / 2
    ...
    def _set_component_dimensions(self):
        """Set dimensions for lid assembly and terminal connectors based on cannister."""
        connector_width = self._cannister._inner_width * 0.3  # 30% of inner width
        connector_length = self._cannister._inner_length * 0.8  # 80% of inner length
        
        self._lid_assembly.width = self._cannister._inner_width * M_TO_MM
        self._lid_assembly.length = self._cannister._inner_length * M_TO_MM
    ...
    def _calculate_internal_dimensions_longitudinal(self):
        ...
        self._internal_height = (
            self._cannister._inner_height - 
            self._lid_assembly._thickness - 
            max_connector_thickness
        )
        self._internal_width = self._cannister._inner_width
        self._internal_length = self._cannister._inner_length
    
    def _calculate_internal_dimensions_transverse(self):
        """Calculate internal dimensions for transverse connector orientation."""
        self._internal_height = (
            self._cannister._inner_width - 
            self._cathode_terminal_connector._thickness - 
            self._anode_terminal_connector._thickness
        )
    ...
    def get_right_left_view(self, **kwargs) -> go.Figure:
        ...
        traces.append(self._cannister.right_left_trace)
    ...
    def get_top_down_view(self, **kwargs) -> go.Figure:
        ...
        traces.append(self._cannister.top_down_trace)
    ...
    @property
    def cannister(self) -> PrismaticCannister:
    ...
    def width(self) -> float:
        ...
        return self._cannister.width
    
    @property
    def length(self) -> float:
        """Outer length of the encapsulation in mm."""
        return self._cannister.length
    ...
    def internal_height(self, internal_height: float) -> None:
        """Set internal height and adjust cannister height accordingly."""
    ...
    def _translate_all_components(self, translation_vector: Tuple[float, float, float]):
        ...
        translation_vector : Tuple[float, float, float]
            Translation in meters (dx, dy, dz)
        """
        self._translate_component(self._cannister, translation_vector)
    ...
    @cannister.setter
    @calculate_all_properties
    def cannister(self, cannister: PrismaticCannister) -> None:
        """Set cannister."""
        self.validate_type(cannister, PrismaticCannister, "Cannister")

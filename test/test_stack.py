...
class TestCellsSingleAM(unittest.TestCase):
    ...
    def test_instantiation(self):
        ...
        self.assertEqual(round(next(iter(self.stack._cathode_mass_breakdown['active_materials'].values())), 5), 
                         round(next(iter(self.stack._cathodes[0]._mass_breakdown['active_materials'].values())) * 26, 5))
        
        self.assertEqual(round(next(iter(self.stack._anode_mass_breakdown['active_materials'].values())), 5), 
                         round(next(iter(self.stack._anodes[0]._mass_breakdown['active_materials'].values())) * 27, 5))
        
        self.assertEqual(round(next(iter(self.stack._cathode_mass_breakdown['conductive_additives'].values())), 5), 
                         round(next(iter(self.stack._cathodes[0]._mass_breakdown['conductive_additives'].values())) * 26, 5))
        
        self.assertEqual(round(next(iter(self.stack._anode_mass_breakdown['conductive_additives'].values())), 5), 
                         round(next(iter(self.stack._anodes[0]._mass_breakdown['conductive_additives'].values())) * 27, 5))
        
        self.assertEqual(round(next(iter(self.stack._cathode_mass_breakdown['binders'].values())), 5), 
                         round(next(iter(self.stack._cathodes[0]._mass_breakdown['binders'].values())) * 26, 5))
        
        self.assertEqual(round(next(iter(self.stack._anode_mass_breakdown['binders'].values())), 5), 
                         round(next(iter(self.stack._anodes[0]._mass_breakdown['binders'].values())) * 27, 5))
        
        # check the cost breakdowns are working
        self.assertEqual(round(next(iter(self.stack._cathode_cost_breakdown['active_materials'].values())), 5), 
                         round(next(iter(self.stack._cathodes[0]._cost_breakdown['active_materials'].values())) * 26, 5))

        self.assertEqual(round(next(iter(self.stack._anode_cost_breakdown['active_materials'].values())), 5),
                         round(next(iter(self.stack._anodes[0]._cost_breakdown['active_materials'].values())) * 27, 5))

        self.assertEqual(round(next(iter(self.stack._cathode_cost_breakdown['conductive_additives'].values())), 5),
                         round(next(iter(self.stack._cathodes[0]._cost_breakdown['conductive_additives'].values())) * 26, 5))

        self.assertEqual(round(next(iter(self.stack._anode_cost_breakdown['conductive_additives'].values())), 5),
                         round(next(iter(self.stack._anodes[0]._cost_breakdown['conductive_additives'].values())) * 27, 5))

        self.assertEqual(round(next(iter(self.stack._cathode_cost_breakdown['binders'].values())), 5),
                         round(next(iter(self.stack._cathodes[0]._cost_breakdown['binders'].values())) * 26, 5))

        self.assertEqual(round(next(iter(self.stack._anode_cost_breakdown['binders'].values())), 5),
                         round(next(iter(self.stack._anodes[0]._cost_breakdown['binders'].values())) * 27, 5))
